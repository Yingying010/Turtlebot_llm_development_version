...
class TransportNode(Node):
    def __init__(self, robot_id: str):
        super().__init__('collab_'+robot_id)
        self.robot_id=robot_id
        self.peer_id="robot1" if robot_id=="robot2" else "robot2"
        self.motion=Motion(self,robot_id)
        ...
        self.pub=self.create_publisher(String,'/transport_coordination',qos)
        self.sub=self.create_subscription(String,'/transport_coordination',self.on_bus,qos)

        # 状态
        self.have_ack=False; self.peer_ready=False; self.self_ready=False; self.start_at=None

        # executor 用于 navigate_to_target
    ...
    # ===== 主流程 =====
    def _main(self):
        self.get_logger().info(f"🧭 Formation phi={math.degrees(self.phi):.1f}° | path={self.path_len:.2f}m")

        # Phase1: 握手
        self._publish(MSG_SYN,{"who":self.robot_id})
        t0=time.time()
        while not self.have_ack and time.time()-t0<3: rclpy.spin_once(self,0.1)
        ...
        self.get_logger().info(f"🚗 {self.robot_id} navigating to {target} ...")
        navigate_to_target(self, self.executor, self.robot_id, target)

        self.self_ready=True; self._publish(MSG_READY,{"who":self.robot_id})
        while not(self.self_ready and self.peer_ready): rclpy.spin_once(self,0.1)
        ...
        if self.is_r1:
            ...
            while self.start_at is None: rclpy.spin_once(self,0.1)

        while self.start_at>time.time(): time.sleep(0.001)
        forward=self.is_r1
        self.get_logger().info(f"🏁 START transport | forward={forward} | dist={self.path_len:.2f}m | v={SPEED}")
...
def main():
    ...
    rclpy.init()
    node=TransportNode(args.robot_id)
    try: rclpy.spin(node)
    except KeyboardInterrupt: pass
    finally: node.motion.stop(); node.destroy_node(); rclpy.shutdown()
