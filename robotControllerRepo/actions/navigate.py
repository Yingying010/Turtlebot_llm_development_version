...
# Publisher 缓存：按 (id(node), robot_name) 隔离，避免跨 Node 复用旧句柄
...
def safe_publish_twist(node: Node, robot_name: str, twist: Twist):
    ...
    对 cmd_vel 发布做保护：若句柄失效则重建后重试一次，避免 InvalidHandle。
