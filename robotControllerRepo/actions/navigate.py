...
def getRobotPositionCache(parent_node: Node, robot_id: str):
    rigid_node = RigidTracker(robot_position_cache, robot_id)
    # ① 把新 node 挂到同一个 executor
    if getattr(parent_node, "_executor", None):
        parent_node._executor.add_node(rigid_node)
    else:                         # 独立脚本模式
        threading.Thread(
            target=rclpy.spin,
            args=(rigid_node,),
    ...
    return rigid_node  # ⬅️ 可以返回节点句柄（可选）
...
def navigate_to_target(node, robot_name, target):
    getRobotPositionCache(node, robot_name)
